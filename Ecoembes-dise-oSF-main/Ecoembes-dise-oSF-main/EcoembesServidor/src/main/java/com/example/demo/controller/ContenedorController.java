...
public class ContenedorController {
        ...
        @Operation(summary = "Obtener todos los contenedores", responses = {
        ...
        @Operation(summary = "Obtener contenedor por ID", responses = {
        ...
        @GetMapping("/{id}")
        public ResponseEntity<ContenedorDTO> getContenedorById(@PathVariable("id") String id) {
        ...
        @Operation(summary = "Crear nuevo contenedor", responses = {
        ...
        @Operation(summary = "Actualizar contenedor", responses = {
                        @ApiResponse(responseCode = "200", description = "Contenedor actualizado"),
                        @ApiResponse(responseCode = "404", description = "Contenedor no encontrado")
        })
        @PutMapping("/{id}")
        public ResponseEntity<ContenedorDTO> updateContenedor(@PathVariable("id") String id,
        ...
        @Operation(summary = "Eliminar contenedor", responses = {
                        @ApiResponse(responseCode = "204", description = "Contenedor eliminado"),
                        @ApiResponse(responseCode = "404", description = "Contenedor no encontrado")
        })
        @DeleteMapping("/{id}")
        public ResponseEntity<Void> deleteContenedor(@PathVariable("id") String id) {
                boolean deleted = ecoembesFacade.deleteContenedor(id);
                return deleted ? ResponseEntity.noContent().build() : ResponseEntity.notFound().build();
        }

        @Operation(summary = "Buscar contenedores por ubicación", responses = {
                        @ApiResponse(responseCode = "200", description = "Lista obtenida") })
        @GetMapping("/ubicacion/{ubicacion}")
        public ResponseEntity<List<ContenedorDTO>> getByUbicacion(@PathVariable("ubicacion") String ubicacion) {
                try {
                        ...
                        e.printStackTrace(); // Esto mostrará el error en la consola del servidor
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
                }
        }

        @Operation(summary = "Buscar contenedores por estado", responses = {
                        @ApiResponse(responseCode = "200", description = "Lista obtenida") })
        @GetMapping("/estado/{estado}")
        public ResponseEntity<List<ContenedorDTO>> getByEstado(@PathVariable("estado") Contenedor.NivelLlenado estado) {
                List<Contenedor> contenedores = ecoembesFacade.getContenedoresByLlenado(estado);
                List<ContenedorDTO> dtos = contenedores.stream().map(this::toDTO).toList();
                return ResponseEntity.ok(dtos);
        }

        @Operation(summary = "Asignar contenedor a una planta", responses = {
        ...
        public ResponseEntity<ContenedorDTO> asignarAPlanta(@PathVariable("id") String id,
        ...
        private ContenedorDTO toDTO(Contenedor c) {
                String nombrePlanta = (c.getPlantaAsignada() != null) ? c.getPlantaAsignada().getNombre()
                                : "SIN ASIGNAR";
                return new ContenedorDTO(
                                ...
                                c.getCodigoPostal(),
                                c.getCapacidadTotal(),
                                ContenedorDTO.NivelLlenado.valueOf(c.getNivelLlenado().name()),
                                c.getNumEnvases(),
        ...
        private Contenedor toEntity(ContenedorDTO dto) {
                ...
                if (dto.getNivelLlenado() != null) {
                        contenedor.setNivelLlenado(Contenedor.NivelLlenado.valueOf(dto.getNivelLlenado().name()));
                }
