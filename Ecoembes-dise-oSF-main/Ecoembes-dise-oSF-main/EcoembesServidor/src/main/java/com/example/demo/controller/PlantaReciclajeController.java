...
public class PlantaReciclajeController {
        ...
        @Operation(summary = "Obtener todas las plantas", responses = {
        ...
        @Operation(summary = "Actualizar planta", responses = {
                        @ApiResponse(responseCode = "200", description = "Planta actualizada"),
                        @ApiResponse(responseCode = "404", description = "Planta no encontrada")
        })
        @PutMapping("/{id}")
        public ResponseEntity<PlantaReciclajeDTO> updatePlanta(@PathVariable("id") Long id,
        ...
        @Operation(summary = "Eliminar planta", responses = {
                        @ApiResponse(responseCode = "204", description = "Planta eliminada"),
                        @ApiResponse(responseCode = "404", description = "Planta no encontrada")
        })
        @DeleteMapping("/{id}")
        public ResponseEntity<Void> deletePlanta(@PathVariable("id") Long id) {
                boolean deleted = ecoembesFacade.deletePlanta(id);
                return deleted ? ResponseEntity.noContent().build() : ResponseEntity.notFound().build();
        }

        @Operation(summary = "Consultar capacidad real en planta externa", description = "Consulta la capacidad directamente a la planta externa (PlasSB o ContSocket)", responses = {
                        @ApiResponse(responseCode = "200", description = "Capacidad obtenida"),
                        @ApiResponse(responseCode = "404", description = "Planta no encontrada")
        })
        @GetMapping("/{id}/capacidad-real")
        public ResponseEntity<CapacidadPlantaDTO> getCapacidadReal(
                        @PathVariable("id") Long id,
                        @RequestParam(name = "date", required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) java.time.LocalDate date) {

                java.time.LocalDate fechaConsulta = date != null ? date : java.time.LocalDate.now();
                CapacidadPlantaDTO capacidad = ecoembesFacade.getCapacidadRealPlanta(id, fechaConsulta);

                return capacidad != null ? ResponseEntity.ok(capacidad) : ResponseEntity.notFound().build();
        }
